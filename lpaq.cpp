...
int main(int argc, char **argv) {
    ...
    if (argc != 4 || (argv[1][0] != 'c' && argv[1][0] != 'd')) {
        printf(
          "To compress:   lpaq c input output\n"
          "To decompress: lpaq d input output\n");
        return 1;
    }
    ...
    // Always use the highest compression level (9)
    MEM = 1 << (9 + 20);  // Force MEM to be the value for option 9
    ...
    if (argv[1][0] == 'c') {
        ...
        fprintf(out, "pQ%c%c%c%c%c%c", 1, '9', 
          size >> 24, size >> 16, size >> 8, size);

        // Compress
        Encoder e(COMPRESS, out);  // Use the COMPRESS enum
        int c;
        long processed = 0;
        while ((c = getc(in)) != EOF) {
            e.compress(c);
            ...
            // Calculate progress as floating-point value for accuracy
            double progress = (double)processed / size * 100;

            // Display progress as a whole number (casting to int for display)
            if (processed % (size / 100) == 0 || processed == 1) {  // Show progress at least once at the start
                printf("\rCompressing: %d%%", (int)progress);  // Display progress as integer
                fflush(stdout);
            }
        }
        e.flush();
        printf("\rCompressing: 100%%\n");
    }
    ...
    // Decompress
        ...
        // Store original file size for percentage calculation
        long originalSize = size;

        // Decompress
        out = fopen(argv[3], "wb");
        if (!out) perror(argv[3]), exit(1);
        Encoder e(DECOMPRESS, in);  // Use the DECOMPRESS enum
        long processed = 0;
        while (size-- > 0) {
            ...
            // Calculate progress as floating-point value for accuracy
            double progress = (double)processed / originalSize * 100;

            // Display progress as a whole number (casting to int for display)
            if (processed % (originalSize / 100) == 0 || processed == 1) {  // Show progress at least once at the start
                printf("\rDecompressing: %d%%", (int)progress);  // Display progress as integer
                fflush(stdout);
            }
        }
        printf("\rDecompressing: 100%%\n");
    }
